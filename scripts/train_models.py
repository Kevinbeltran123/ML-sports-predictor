"""Train XGBoost + CatBoost moneyline models from dataset.sqlite.

Uso:
    PYTHONPATH=. python scripts/train_models.py
    PYTHONPATH=. python scripts/train_models.py --dataset dataset_2012-26
"""
...
def main():
    parser = argparse.ArgumentParser(description="Train XGBoost + CatBoost ML models")
    parser.add_argument("--dataset", default=DEFAULT_DATASET)
    args = parser.parse_args()

    print(f"\n{'='*65}")
    print(f"  TRAIN XGBoost + CatBoost — Moneyline")
    print(f"{'='*65}")

    df = load_and_prepare(args.dataset)

    # Temporal split: train < 2025-10-01, test >= 2025-10-01
    ...
    # --- 4. Save models ---
    print(f"\n  Saving models to {NBA_ML_MODELS_DIR}...")
    ...
    # Use last 20% of train as calibration set
    ...
    # Compute ensemble probs on calibration set
    ...
    # --- 6. Variance model (simple: std of component predictions) ---
    p_ens_test = p_ensemble[:, 1]
    variance_info = {
