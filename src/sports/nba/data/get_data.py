...
from src.config import CONFIG_PATH, TEAMS_DB as DB_PATH, get_logger
...
def backfill_season(con, url, season_key, value, existing_dates, today):
    ...
    if not missing_dates:
        logger.info("No missing dates for season %s.", season_key)
        return

    logger.info("Backfilling %d dates for season %s.", len(missing_dates), season_key)
    for date_pointer in missing_dates:
        logger.info("Getting data: %s", date_pointer)
        df = fetch_data(url, date_pointer, value["start_year"], season_key)
        if df.empty:
            logger.warning("No data returned for: %s", date_pointer)
            continue
    ...
    with sqlite3.connect(db_path) as con:
        ...
        if backfill:
            ...
            if season:
                ...
                if not season_items:
                    logger.error("Season not found in config: %s", season)
                    return
        ...
        if not season_key:
            logger.warning("No current season found for today: %s", today)
            return
        ...
        if fetch_start > fetch_end:
            logger.info("No new dates to fetch. Latest date: %s", latest_date)
            return
