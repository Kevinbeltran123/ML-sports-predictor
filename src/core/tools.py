...
def create_todays_games_from_odds(input_dict, team_mappings):
    """Filtra juegos del dict de odds, dejando solo equipos validos.

    Args:
        input_dict: dict con keys "HomeTeam:AwayTeam" (viene de odds API)
        team_mappings: dict de nombres de equipo validos (ej: team_index_current)
    """
    games = []
    for game in input_dict.keys():
        home_team, away_team = game.split(":")
        if home_team not in team_mappings or away_team not in team_mappings:
            continue
        games.append([home_team, away_team])
    return games
